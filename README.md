...
#Read filtering is important for removing adaptors and low quality reads
#Trim galore is great as it autodetects adaptors and the default settings should be good enough for most applications
module load trimgalore/0.6.6 #we are assuming paired-end reads
trim_galore --paired sample1_1.fq sample1_2.fq --output_dir sample1 #This will write files named sample1_1_trimmed.fq and sample1_2_trimmed.fq into aa directory named sample1
...
#sample A1 ran for 45 min
#Key function here => trim_galore --paired sample1_1.fq sample1_2.fq --output_dir sample1
...
#Use the ‘nano’ function to build .sh (saved as .sbrun) on terminal interface 
...
module load salmon/1.8.0
salmon quant -i salmon_index -l IU -1 sample1_1_trimmed.fq -2 sample1_2_trimmed.fq --validateMappings -o transcripts_quant #this is the skeleton function to quant each sample
#This will produce a folder named transcripts_quant which has your file called quant.sf which is the quantification file
...
salmon quant -i /data/lab/phelps/Max_O_gorb_reads/salmon_index -l IU -1 ./sampleR1/R1_1_val_1.fq.gz -2 ./sampleR1/R1_2_val_2.fq.gz --validateMappings -o script_quant_R1 #this is my modified code showing adjustments for location of files
...
grep -c ID=rna*.* GCFOgo.gff #pull RNA line
...
files <- file.path("RNA_quant_files", sample_names$X1, "quant.sf") #double check location and naming
...
#make a new meta .csv for each differential comparison we make - ‘_meta.csv’
#[Sample,Treatment]
#[sample#,Control/Test]
library(readr)
...
contrast <- c("Treatment", “Test”, “Control")
...
#First we will concatenate differential expression to emulate migration transitions
library("dplyr")
#as a group of 3 sequences:
...
#supplemental [maleGene_’tissue’-pattern.csv] present the genes that fall into each expression pattern, compiled together.
...
#Try out inputs on their web browser : I used .gtf.gz ncbi file as proteins, and selected ‘Protein’, or even FASTA files
...
#Next, I extracted the list of genes that fit the pattern and fall into ‘Immune Effector Process’
...
#From here, I used the ‘intersect()’ function to find similar genes across tissues, narrowing down to 31 found across all tissues sampled
